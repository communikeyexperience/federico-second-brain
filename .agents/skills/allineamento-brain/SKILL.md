...
3. Lo script legge ricorsivamente tutte le note del brain (esclude `.obsidian`, `.Codex`, `.git`, `Templates`, `node_modules`, e ogni nota che inizia per `ELIMINA_`), le invia in batch a Voyage AI (modello `voyage-4`) e scrive il risultato in `data/vault-index.json`.
